...
function initInputObserver(cb, blockClass, ignoreClass, maskInputOptions, maskInputFn, sampling) {
    ...
    function cbWithDedup(target, v) {
        ...
        if (!lastInputValue ||
            ...
            var id = mirror.getId(target);
            cb(__assign$1(__assign$1({}, v), { id: id }));
        }
    }
...
function wrapEvent(e) {
    return __assign$1(__assign$1({}, e), { timestamp: Date.now() });
}
var wrappedEmit;
            ...
            handlers_1.push(initObservers({
                mutationCb: function (m) {
                    return wrappedEmit(wrapEvent({
                        ...
                        data: __assign$1({ source: IncrementalSource.Mutation }, m),
                    }));
                },
                ...
                mouseInteractionCb: function (d) {
                    return wrappedEmit(wrapEvent({
                        type: EventType.IncrementalSnapshot,
                        data: __assign$1({ source: IncrementalSource.MouseInteraction }, d),
                    }));
                },
                scrollCb: function (p) {
                    return wrappedEmit(wrapEvent({
                        type: EventType.IncrementalSnapshot,
                        data: __assign$1({ source: IncrementalSource.Scroll }, p),
                    }));
                },
                viewportResizeCb: function (d) {
                    return wrappedEmit(wrapEvent({
                        type: EventType.IncrementalSnapshot,
                        data: __assign$1({ source: IncrementalSource.ViewportResize }, d),
                    }));
                },
                inputCb: function (v) {
                    return wrappedEmit(wrapEvent({
                        type: EventType.IncrementalSnapshot,
                        data: __assign$1({ source: IncrementalSource.Input }, v),
                    }));
                },
                mediaInteractionCb: function (p) {
                    return wrappedEmit(wrapEvent({
                        type: EventType.IncrementalSnapshot,
                        data: __assign$1({ source: IncrementalSource.MediaInteraction }, p),
                    }));
                },
                styleSheetRuleCb: function (r) {
                    return wrappedEmit(wrapEvent({
                        type: EventType.IncrementalSnapshot,
                        data: __assign$1({ source: IncrementalSource.StyleSheetRule }, r),
                    }));
                },
                canvasMutationCb: function (p) {
                    return wrappedEmit(wrapEvent({
                        type: EventType.IncrementalSnapshot,
                        data: __assign$1({ source: IncrementalSource.CanvasMutation }, p),
                    }));
                },
                fontCb: function (p) {
                    return wrappedEmit(wrapEvent({
                        type: EventType.IncrementalSnapshot,
                        data: __assign$1({ source: IncrementalSource.Font }, p),
                    }));
                },
                logCb: function (p) {
                    return wrappedEmit(wrapEvent({
                        type: EventType.IncrementalSnapshot,
                        data: __assign$1({ source: IncrementalSource.Log }, p),
                    }));
                },
                blockClass: blockClass,
                blockSelector: blockSelector,
...
var TrackSession = (function () {
    function TrackSession(options) {
        this.domain = 'http://127.0.0.1:7008';
        this.recordNSP = 'record';
        this.liveNSP = 'live';
        this.storeKey = 'tracksession-';
        this.options = options;
        this.init();
    }
    TrackSession.addCustomEvent = function (tag, paylaod) {
        if (TrackSession.isCertified && record) {
            record.addCustomEvent(tag, paylaod);
        }
    };
    TrackSession.prototype.init = function () {
        this.verifyToken();
    };
    TrackSession.prototype.request = function (url, body) {
        return __awaiter(this, void 0, void 0, function () {
            var response, json;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4, fetch("" + this.domain + url, {
    ...
    TrackSession.prototype.verifyToken = function () {
        return __awaiter(this, void 0, void 0, function () {
            var _a, token, pageUrl, screenWidth, screenHeight, resolution, userId, sessionId, bodyParams, json, code, _b, body, userId_1, sessionId_1;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        _a = this.options.token, token = _a === void 0 ? '' : _a;
                        if (!token)
                        ...
                        userId = this.getUserByType('user');
                        sessionId = this.getUserByType('session');
                        bodyParams = {
                            token: token,
                            userId: userId,
                            sessionId: sessionId,
                        ...
                        return [4, this.request('/verifytoken', bodyParams)];
                    case 1:
                        json = _c.sent();
                        code = json.code, _b = json.body, body = _b === void 0 ? {} : _b;
                        if (code === 200) {
                            userId_1 = body.userId, sessionId_1 = body.sessionId;
                            TrackSession.isCertified = true;
                            this.sessionId = sessionId_1;
                            this.userId = userId_1;
                            this.setUserByType('user', userId_1);
                            this.setUserByType('session', sessionId_1);
                            this.openSocket();
    ...
    TrackSession.prototype.onDocumentVisibilityState = function () {
        return __awaiter(this, void 0, void 0, function () {
            var updateHandle;
            var _this = this;
            return __generator(this, function (_a) {
                updateHandle = function (visibilityState) { return __awaiter(_this, void 0, void 0, function () {
                    var sessionId, bodyParams;
                    return __generator(this, function (_a) {
                        switch (_a.label) {
                            case 0:
                                sessionId = this.getUserByType('session');
                                ...
                                return [4, this.request('/updatevisibilitystate', bodyParams)];
                            case 1:
                                _a.sent();
                                return [2];
                        }
    ...
    TrackSession.prototype.getUserByType = function (type) {
        var token = this.options.token;
        try {
            var key = this.storeKey + "-" + type + "id-" + token;
            if (type === 'session') {
    ...
    TrackSession.prototype.setUserByType = function (type, userid) {
        var token = this.options.token;
        try {
            var key = this.storeKey + "-" + type + "id-" + token;
            if (type === 'session') {
    ...
    TrackSession.prototype.initSocketEvent = function () {
        var _this = this;
    ...
    TrackSession.prototype.initLiveSocketEvent = function () {
        var _this = this;
        this.socketLive.on('connect', function () {
            console.log('connect*****');
            _this.startLiveRecord();
        });
        this.socketLive.on('reconnect', function () {
            _this.restartLiveRecord();
        });
        this.socketLive.on('connect_error', function (error) {
            console.log('connect_error', error);
        });
        this.socketLive.on('connect_timeout', function (timeout) {
            console.log('connect_timeout', timeout);
        });
        this.socketLive.on('disconnect', function (reason) {
            console.log('disconnect', reason);
            _this.stopRecord();
            if (reason === 'io server disconnect') {
                _this.socketLive.connect();
            }
        });
        this.socketLive.on('online', function (msg) {
            console.log('msg', msg);
            _this.restartLiveRecord();
        });
    };
    TrackSession.prototype.openSocket = function () {
        var token = this.options.token;
        var url = this.domain + "/" + this.recordNSP;
        var liveUrl = this.domain + "/" + this.liveNSP;
        var option = {
            token: token,
    ...
    TrackSession.prototype.startRecord = function () {
        var _this = this;
        var recordOptions = omit(this.options, ['token']);
        this.recordStopFn = record(__assign({ emit: function (event) {
                console.log('record', event);
                _this.socket.emit(_this.recordNSP, event);
            } }, recordOptions));
    };
    TrackSession.prototype.stopRecord = function () {
        if (this.recordStopFn) {
            this.recordStopFn();
        }
    };
    TrackSession.prototype.startLiveRecord = function () {
        var _this = this;
        var recordOptions = omit(this.options, ['token']);
        this.liveStopFn = record(__assign({ emit: function (event) {
                console.log('live', event);
                _this.socketLive.emit(_this.liveNSP, event);
            } }, recordOptions));
    };
    TrackSession.prototype.stopLiveRecord = function () {
        if (this.liveStopFn) {
            this.liveStopFn();
        }
    };
    TrackSession.prototype.restartLiveRecord = function () {
        this.stopLiveRecord();
        this.startLiveRecord();
    };
    TrackSession.prototype.restartRecord = function () {
        this.stopRecord();
        this.startRecord();
    };
    TrackSession.isCertified = false;
    return TrackSession;
}());

(function () {
    var options = { token: window['trackSessionToken'] || '' };
